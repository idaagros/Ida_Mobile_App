...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  // Lays out a section's tiles in a single column on mobile (today's
  // unchanged behavior) or a reflowing multi-column grid on wider
  // screens. Each tile is wrapped in a fixed-width SizedBox before
  // going into the Wrap - _tile() itself uses Expanded internally,
  // which needs a bounded width to work; Wrap alone gives children
  // unbounded width and would crash without this.
  Widget _tileGrid(BuildContext context, List<Widget> tiles) {
    final columns = Responsive.gridColumns(context);
    if (columns == 1) return Column(children: tiles);
    const spacing = 12.0;
