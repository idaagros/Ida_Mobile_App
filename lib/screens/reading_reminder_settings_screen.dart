...
class ReadingReminderSettingsScreen extends StatefulWidget {
  ...
  @override
  State<ReadingReminderSettingsScreen> createState() => _ReadingReminderSettingsScreenState();
}

class _ReadingReminderSettingsScreenState extends State<ReadingReminderSettingsScreen> {
  ...
  String _formatForApi(TimeOfDay t) => '${t.hour.toString().padLeft(2, '0')}:${t.minute.toString().padLeft(2, '0')}';

  Future<void> _load() async {
    setState(() { loading = true; error = null; });
    try {
      final h = await _headers;
      final res = await http.get(Uri.parse('$baseUrl/reading-reminder-settings'), headers: h);
  ...
  Future<void> _pickTime(bool isUser) async {
    final picked = await showTimePicker(context: context, initialTime: isUser ? userTime : adminTime);
    if (picked != null) {
      setState(() {
        if (isUser) {
          userTime = picked;
  ...
  Widget _timeRow(String title, String description, TimeOfDay time, bool isUser) {
    return InkWell(
        ...
        child: Row(children: [
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(title, style: const TextStyle(fontSize: 13.5, fontWeight: FontWeight.w700, color: idaDark)),
          ...
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFFF4F7F2), borderRadius: BorderRadius.circular(8)),
            child: Text(time.format(context), style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: idaGreen)),
          ),
        ]),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Reminder Settings', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
      ),
      body: loading
