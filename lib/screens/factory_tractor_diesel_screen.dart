...
class FactoryTractorDieselScreen extends StatefulWidget {
  ...
  @override
  State<FactoryTractorDieselScreen> createState() =>
      _FactoryTractorDieselScreenState();
}

class _FactoryTractorDieselScreenState
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (results[0].statusCode == 200) {
        setState(() => logs =
            List<Map<String, dynamic>>.from(jsonDecode(results[0].body)));
      }
      if (results[1].statusCode == 200) {
        final avgData = jsonDecode(results[1].body);
        setState(() {
          averageLPerHour =
              (avgData['average_liters_per_hour'] as num?)?.toDouble();
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          title: const Text('Log Diesel Fill-up',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Factory Tractor — Diesel',
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(children: [
                          ...
                          Text(
                            averageLPerHour != null
                                ? '${averageLPerHour!.toStringAsFixed(2)} L/hr'
                                : '—',
                            style: const TextStyle(
                                fontSize: 28,
                                fontWeight: FontWeight.w700,
                      ...
                      if (logs.isEmpty)
                        Padding(
                          ...
                          child: Center(
                              child: Text(
                                  'No diesel logs yet — tap + to add one.',
                                  style: TextStyle(
                                ...
                                Expanded(
                                  child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                            '${log['liters_filled']} L${log['cost'] != null ? ' · ₹${log['cost']}' : ''}',
                                            style: TextStyle(
                                                fontSize: 12.5,
                                ...
                                if (canEdit && log['status'] == 'pending') ...[
                                  IconButton(
                                      icon: const Icon(
                                          ...
                                          color: idaGreen),
                                      onPressed: () =>
                                          _updateStatus(log['id'], 'approved')),
                                  IconButton(
                                      icon: const Icon(Icons.cancel_outlined,
                                          color: Color(0xFFC0392B)),
                                      onPressed: () =>
                                          _updateStatus(log['id'], 'returned')),
                                ],
                              ]),
                            )),
                    ],
